...
public class ContentUtils {
    ...
    public static void provideAssets(Activity activity, String cluster_id) {
        documentsProvided.clear();
        try {
            ...
            /*add_data = MyApplication.Companion.get().getDb().filePathMain().all(cluster_id);

            for (int j = 0; j < add_data.size(); j++) {

                String[] path=add_data.get(j).getLocal_pathImage().split("/");
                ArrayUtils.reverse(path);
                documentsProvided.put(path[0].replace(".obj",".mtl"), Uri.parse(add_data.get(j).getLocal_pathImage().replace(".obj",".mtl")));
            }*/
    ...
    public static void provideAssets_i_m_here(Activity activity) {
        documentsProvided.clear();
        try {

           // for (String document : activity.getAssets().list("models")) {
                documentsProvided.put("source_i_m_here.mtl", Uri.parse("assets://assets/models/" + "source_i_m_here.mtl"));
                documentsProvided.put("pointer_shadow_patch.mtl", Uri.parse("assets://assets/models/" + "pointer_shadow_patch.mtl"));
           // }
